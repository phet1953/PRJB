...
function SaveEdit()
{
    ...
    $strsql = "UPDATE breeder 
    SET No_b='" . $num . "',dad_b='" . $num1 . "',breed_b='" . $selectAedit . "'
    ,mom_b='" . $num2 . "',birthfrom_b='" . $datfromedit . "',form_b='" . $noedit . "',birthday_b='" . $datbirthedit . "'
...
function Changfunction()
{
    ...
    include '../connect/connect.php';
    $strsql = "SELECT * FROM `breeder` WHERE Isdelete = 0 ORDER BY No_b ASC ";
    $query = mysqli_query($conn, $strsql);
    if (mysqli_num_rows($query) > 0) {
        ...
        <thead>
         <tr>
          <th bgcolor='#ffe6ee' class='col-md-1 col-xs-1'><center>เบอร์หูสุกรแม่พันธุ์</center></th>
          <th bgcolor='#ffe6ee' class='col-md-2 col-xs-1'><center>สายพันธ์ุ</center></th>
          <th bgcolor='#ffe6ee' class='col-md-2 col-xs-1'><center>เบอร์หูพ่อของสุกรแม่พันธู์</center></th>
          <th bgcolor='#ffe6ee' class='col-md-2 col-xs-1'><center>เบอร์หูแม่ของสุกรแม่พันธู์</center></th>
        ...
        while ($re = mysqli_fetch_assoc($query)) {
            ...
            echo "<td>
            <button type='button' class='btn btn-lg' style='background-color: white;margin-bottom:-5px;margin-top: -8px;width: 0px; height: 0px;' data-toggle='modal' data-target='#flipFlop$re[ID_breeder]'><img src='https://img.icons8.com/dusk/24/000000/edit-property.png'></button>
            <input type='image' id='delete' style='margin-bottom: -16px;' onclick=mom_delete('" . $re['ID_breeder'] . "') src='https://img.icons8.com/plasticine/26/000000/full-trash.png'>
            </td>";
            echo "</tr>";
            echo "<div class='modal fade' id='flipFlop$re[ID_breeder]' tabindex='-1' role='dialog' aria-labelledby='modalLabel' aria-hidden='true'>
                ...
                <div class='row'>
                    <div class='col-md-12'>
                        <div class='col-md-4 form-group' align='center'>
                            <label>แก้ไข เบอร์หูสุกรแม่พันธู์</label>
                            <input class='form-control' name='number' id='numberedit$re[ID_breeder]' type='number' style='height:40px; font-size:16px;' value='$num'>
                        </div>
                        <div class='col-md-4 form-group' align='center'>
                            <label>แก้ไข เบอร์หูพ่อของสุกรแม่พันธู์</label>
                            <input class='form-control' name='det' id='detedit$re[ID_breeder]' type='number' style='height:40px; font-size:16px;' value='$num1'>
                        </div>
                        <div class='col-md-4 form-group' id='select_option' align='center'>";

            $strsql2 = "select breed_b from breed";
            $query2 = mysqli_query($conn, $strsql2);
            if (mysqli_num_rows($query2) > 0) {
                echo "<label>แก้ไข สายพันธ์ุ</label>
                                <select id='selectAedit$re[ID_breeder]' name='selectA' class='form-control' style='height:40px; font-size:16px;padding: 0px 60px;'>
                                <option value=''>เลือกรายการ</option>";
            ...
            echo "</div>
                        <div class='col-md-4 form-group' align='center'>
                            <label>แก้ไข เบอร์หูแม่ของสุกรแม่พันธู์</label>
                            <input class='form-control' name='mom' id='momedit$re[ID_breeder]' type='number' style='height:40px; font-size:16px;' value='$num2'>
                        </div>
                        <div class='col-md-4 form-group' align='center'>
                            <label>แก้ไข วันที่เข้าฝูง</label>
                            <input type='date' class='form-control' name='datfrom' id='datfromedit$re[ID_breeder]' style='height:40px; font-size:16px;padding: 0px 60px;' value='$re[birthfrom_b]'>
                        </div>
                        <div class='col-md-4 form-group' align='center'>
                            <label>แก้ไข แหล่งที่มา</label>
                            <input class='form-control' name='no' id='noedit$re[ID_breeder]' type='text' style='height:40px; font-size:16px;' value='$re[form_b]'>
                        </div>
                        <div class='col-md-4 form-group' align='center'>
                            <label>แก้ไข วันที่เกิด</label>
                            <input type='date' class='form-control' name='datbirth' id='datbirthedit$re[ID_breeder]' style='height:40px; font-size:16px;padding: 0px 60px;' value='$re[birthday_b]'>
                        </div>
                        <br>
                    </div>
                </div>
            ...
            <div class='modal-footer'>
                <button type='button' class='btn btn-secondary' data-dismiss='modal' onclick=Saveedit('$re[ID_breeder]')>บันทึก</button>
...
function inser_mom()
{
    ...
    $strsql = "SELECT * FROM `breeder` WHERE No_b = '" . $num . "'";
    $query = mysqli_query($conn, $strsql);
    if (mysqli_num_rows($query) > 0) {
        echo json_encode(array('status' => '0', 'message' => 'fail ซ้ำเบอร์หูสุกรแม่พันธู์'));
    } else {
        $strsql1 = "SELECT * FROM `breederdad` WHERE No_b = '" . $num . "'";
        $query1 = mysqli_query($conn, $strsql1);
        if (mysqli_num_rows($query1) > 0) {
            echo json_encode(array('status' => '0', 'message' => 'fail ซ้ำเบอร์หูสุกรพ่อพันธู์'));
        } else {
            $strsql2 = "INSERT INTO breeder (No_b, dad_b, mom_b, birthday_b, birthfrom_b, breed_b, form_b) VALUE('" . $num . "','" . $num1 . "','" . $num2 . "','" . $datbirth . "','" . $datfrom . "','" . $select_option . "','" . $no . "')";
            $query2 = mysqli_query($conn, $strsql2);
            if ($query2) {
